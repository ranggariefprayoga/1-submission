...
import {
  AiFillLike, AiFillDislike, AiOutlineLike, AiOutlineDislike,
} from "react-icons/ai";
      ...
      <CommentItemUserContainer>
        ...
        <VoteContainer>
          <UpVote onClick={handleUpVoteComment}>
            {comment.upVotesBy.includes(authUser.id) ? <AiFillLike /> : <AiOutlineLike />}
            {' '}
            <strong>{comment.upVotesBy.length}</strong>
          </UpVote>
          <DownVote onClick={handleDownVoteComment}>
            {comment.downVotesBy.includes(authUser.id) ? <AiFillDislike /> : <AiOutlineDislike />}
            {' '}
            <strong>{comment.downVotesBy.length}</strong>
          </DownVote>
        </VoteContainer>
      </CommentItemUserContainer>
